...
- **El Panel del Guardián** ([guía](https://github.com/domfortunato/air-bladder/blob/master/docs/warden-dashboard.md)) — las 45 tablas de tiradas del Guardián, cada una en un botón, en una sola ventana redimensionable y desacoplable: viajes y clima, encuentros por terreno, PNJ, facciones y monstruos. Elige quién ve cada resultado, tira un conjunto entero en una sola tarjeta, y accede a los generadores y al Daño del Guardián desde el mismo sitio
- **Las guardias, contadas por ti** ([guía](https://github.com/domfortunato/air-bladder/blob/master/docs/keeping-time.md)) — en Cairn el día se divide en tres guardias, y la guardia actual aparece en la pantalla de todos, sobre la lista de jugadores. El Guardián mueve el reloj desde el Panel, y **El clima de hoy** tira la tabla de la estación en la que esté el mundo. El calendario propio de Vald —doce meses, semana de seis días y las estaciones Muerta, Seca, Húmeda y de Cosecha, cada una con su tabla de clima— está disponible como un hack opcional, con un calendario mensual que cualquiera puede abrir desde el reloj
...
La interfaz está traducida al **español** (el 68 % de las cadenas actuales, por [Malecho](https://github.com/fsmalecho)), y el *contenido* del juego —trasfondos, objetos, hechizos, tablas— está traducido únicamente al español.
