...
- [Random character generation](https://github.com/domfortunato/air-bladder/blob/master/docs/generating-characters.md) from Cairn 2e, **Custom Cairn 2e**, and Cairn Barebones backgrounds — each source toggles on or off, and the sheet's **Character Creation Mode** re-rolls single parts of the result. Rolled at the table instead? Clear **Use random generation** and the same button opens an empty sheet, with the pick-lists ready. An empty sheet is never handed anything: your choices are recorded, and you type the inventory you already have
- **The Warden's Dashboard** ([guide](https://github.com/domfortunato/air-bladder/blob/master/docs/warden-dashboard.md)) — every one of the 45 Warden roll tables on a button, in one resizable, detachable window: travel and weather, encounters by terrain, NPCs, factions and monsters. Rolls stay private to you unless you say otherwise, a whole set draws into one card, and reach the generators and Warden's Damage from the same place
- **Watches, kept for you** ([guide](https://github.com/domfortunato/air-bladder/blob/master/docs/keeping-time.md)) — a day is three watches in Cairn, and the current one sits on everyone's screen above the player list. The Warden moves the clock from the Dashboard, and **Roll Today's Weather** rolls the table for whatever season the world is in. Vald's own calendar — twelve months, a six-day week, the Dead, Dry, Wet and Harvest seasons with a weather table each — is there as an optional hack, with a month calendar anyone can open from the clock
...
- Custom 2e backgrounds — seven ship in the **Backgrounds (Custom)** compendium; [create and lint your own](https://github.com/domfortunato/air-bladder/blob/master/docs/creating-custom-backgrounds.md), [share them across worlds](https://github.com/domfortunato/air-bladder/blob/master/docs/sharing-custom-backgrounds.md), or press **Create Custom 2e Backgrounds and Tables…** once and every shipped background, bond, omen, scar and trait table is yours to edit, or hide any of the shipped ones with an eye toggle
- Custom bonds, and custom everything else — a world table named **Bonds** replaces the shipped 2e table for canon and custom backgrounds alike, a custom background can name a table of its own, and the same rule covers every table the generators roll: traits, NPC and monster tables, Barebones creation, Scars, the random-spell table; [customizing bonds](https://github.com/domfortunato/air-bladder/blob/master/docs/customizing-bonds.md)
- A customizable **marketplace** — press **Create a Custom Marketplace…** once and the whole shop is yours to edit, or a world table named **Market: Gear** (or Weapons, Armor, Transports & Containers) replaces that aisle and a name of your own adds one. Every price is read off the item itself, so re-pricing the shop is editing one number, and your shop survives every update; [customizing the marketplace](https://github.com/domfortunato/air-bladder/blob/master/docs/customizing-the-marketplace.md)
...
- Three portrait-picker galleries: 80 character portraits by [Jon Aspeheim](https://jonaspeheim.itch.io/), 368 creature & NPC tokens by [tlomdev](https://tlomdev.itch.io/) (an imported Kettlewright character keeps its face), and 22 monsters and 31 characters by [Lydia Comer](https://linktr.ee/lydiadidmyink)
...
The interface is translated into **Spanish** (61% of the current strings, by [Malecho](https://github.com/fsmalecho)), and game *content* — backgrounds, items, spells, tables — is translated into Spanish only.

Danish, French, German, Polish and Brazilian Portuguese interface files are inherited from the original Cairn system. They cover **about 5%** of the current interface, predate most of this system's features, and are **not actively maintained** — a game in those languages is mostly English in practice. Anything untranslated falls back to English string by string, so a partial translation is always usable rather than broken.
...
- **Game text — CC BY-SA 4.0.** The Cairn rules and text (1st and 2nd edition) are by **Yochai Gal**, licensed [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/). 2e content in this system inherits that license; derivatives must share alike and attribute Yochai Gal. That includes the rules text shipped as readable pages: the **Cairn 2e Rules** compendium holds "Overview & Principles" and "Core Rules for Players", transcribed from the 2e text, and the **Vald** compendium holds the Warden's Guide setting chapter of the same name, together with the 24 named festivals of its calendar, both fetched from the Cairn SRD. The hireling catalogue the generator reads, `module/npc-careers-2e.json`, is game text too, and so are the rules tooltips in `lang/en.json` that quote 2e's own wording. **Cairn's text is CC BY-SA 4.0 wherever it sits:** a directory named under the code license does not change what is inside its files, and MIT never reaches it.
- **This README — CC BY-SA 4.0.** `README.md` and `README.es.md` ship inside the release archive, and both summarise and quote the Cairn 2e text they credit, so they travel under the game-text licence rather than the MIT code grant. Until 2026-09-11 no clause named them at all, which — under the Code clause's "these and only these" — left two shipped documents with no licence.
...
- **Air Bladder logo — © Lydia Comer, all rights reserved. Monster and character art — CC BY-NC-SA 4.0.** Everything in `art/lydia-comer/` is by **[Lydia Comer](https://linktr.ee/lydiadidmyink)**, under two licences. The galleries — 22 creatures and 31 characters (source: [domfortunato.itch.io/lydia-comer](https://domfortunato.itch.io/lydia-comer)), each a square portrait paired with the circle-cropped token drawn from it, offered in the portrait picker's **Lydia Comer** tab (her characters on Player Character, NPC and Hireling sheets; her monsters on Monster sheets) — are licensed [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/): redistribution and adaptation are fine for **noncommercial use**, attribution is required, adaptations must be shared under the same licence, and the artist states no AI was used. **Modified: re-encoded to WebP q95, the token halves sized to 400×400 for the canvas, and changed in no other way** — the indication CC BY-NC-SA 4.0 §3(a)(1)(B) requires. The **logo** files at the top of the folder stay **all rights reserved**, licensed to the Air Bladder system for inclusion and redistribution as part of the system and its forks, and for use representing and promoting the project; any use outside the project requires permission. Contents are listed in `art/lydia-comer/CREDITS.md`; both sets of terms are in `art/lydia-comer/license.txt`.
