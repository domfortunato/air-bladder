...
as of 2026-09-10, measured against the last release (`0.1.20`).
...
| `CAIRN.Notify.OfferNoTargets` | There is no other character to give this to. |
| `CAIRN.Notify.OfferNotYours` | Only {target}'s player (or the Warden) can answer this offer. |
...
| `CAIRN.Offer.DragBody` | {target} belongs to another player, so this can't be a straight hand-over. Post an offer instead? {item} stays with {giver} until it is accepted. |
| `CAIRN.Offer.DragTitle` | Offer {item}? |
| `CAIRN.Offer.GiveHint` | Give to another character… |
| `CAIRN.Offer.OverburdenBody` | Taking {item} leaves {target} with no free slot. An encumbered character's Hit Protection is 0 until something is dropped or handed away. Accept anyway? |
