...
| `npm run dev:item-pile` | the container class label and Item Piles. Walks **every** shipped transport asserting its label and its art agree (they come from one classifier, so drift shows as "Horse" beside a picture of a cart), then makes a pile — an npc whose **variety** is `pile`, since a pile is neither a type nor a kind any more: class art on creation, re-arting when the variety changes, hand-picked art left alone, the sheet's Variety control (value, datalist options, unclipped), directory visibility, and — **as Alice** — that a pile refuses a player with no rights and works once granted them. Rewriting this leg off the retired `transportKind` pick-list is what exposed the `migrateData`-over-diffs bug now gated by `dev:role-migration` |
