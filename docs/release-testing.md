...
| `npm run dev:header-buttons` | Roll Character and the Randomization toggle as inline title-bar buttons: labels, state (the toggle hides Roll Character and relabels itself), that an unrelated re-render leaves them alone, and that neither label is clipped or escapes the header |
