...
| `npm run dev:role-migration` | the `role` migration (docs/npc-roles-plan.md), i.e. that a world upgraded from before the role field comes out stamped — hirelings as hirelings, day rate still on screen — in both of its layers. **The shim** (`NpcData.migrateData`) is asserted on unsaved constructions: `forHire:true` → hireling, `inanimate` → transport/container by class, else npc — **and a Kind ALONE must NOT derive**, which is the fix of 2026-07-31. `migrateData` runs over update DIFFS as well as whole sources (measured: `migrateData({containerClass:"pile"})` comes back carrying `role:"npc"`), so a guard of the shape "field X is absent" fires on every update that simply did not mention X. The old guard included `|| source.containerClass`, so writing a Kind and nothing else re-derived a role from `inanimate` — a key no modern document has — and answered "npc": setting a crate's Kind demoted it to a plain NPC, and so did **picking its art**, because `_setContainerArt` writes exactly `{"system.containerClass": cls}`. The sheet was the only safe path, since its role `<select>` rides along in the submit, which is why every manual test missed it. Two legs guard it now: the diff must come back with no `role` key, and an art-picker-shaped update on a role-container npc must leave the role standing. **The one-time stamp** needs the real `ready`-hook path, so the probe seeds pre-migration actors and **reloads**; a fresh-world validation cannot see this class of defect by construction. Seeding is deliberately awkward now — the schema strips the legacy fields from any write, so "pre-migration" is seeded by DELETING the stored role (`system.-=role`) and the probe asserts that landed before trusting anything downstream. Asserts a pre-fold `hireling`-TYPE doc (which stores none of the legacy fields; only its type says what it is) comes out `role: hireling` with its rate intact and the day-rate row rendered, an npc carrying a bare day rate does too, a plain npc is stamped `npc` — an over-broad migration would grow day-rate rows across the bestiary — and the migration **names itself in the log** as the writer. **Then it re-roles that npc to Monster and reloads again, and the choice must stick**: the migration is one-shot on a stored marker (`roles-migrated`), not on the state it writes, which is the trap its `forHire` predecessor fell into with its checkbox |
