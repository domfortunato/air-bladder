...
| `npm run dev:container-link` | container linking **as a real player** — a GM passes every ownership check *and* every Warden gate, so only this catches permission bugs. A player dropping a Warden-owned mule onto their own sheet must be refused, told why, and leave both documents untouched; a legitimate purchase must come out fully linked and owned; the Connections row must offer that player **neither** trash nor unlink (Round 2 made edge management the Warden's) while the same row on the Warden's screen offers both — without that half the player assertions are satisfied by a row that renders nothing. Ends on the derived list: `connectedTo` alone puts an actor on the tab, clearing it removes it, deleting it makes it vanish, and **`system.containers` must not exist in the schema** — a stronger statement than the "the legacy array is empty" it replaced, since it fails if anyone re-adds the field |
...
document whose sheet quietly loses a row.
