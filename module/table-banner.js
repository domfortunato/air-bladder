 ...
 *   any other pack table of ours      → templateImport
...
export const abTableRoleBanner = (app, element, context, options) => {
  ...
  // names the formula and the tab where it is set.
  values.dice = hasFlatDie(app.document)
    ? L(KEYS.newRow)
    : game.i18n.format(KEYS.customDice, { formula: esc(String(app.document._source?.formula ?? "").trim()) });
  ...
  p.innerHTML = `<i class="fa-solid fa-circle-info"></i><span>${game.i18n.format(KEYS[role.role], values)}</span>`;
  table.before(p);
